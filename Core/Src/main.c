...
/* USER CODE BEGIN PV */
LoRa myLoRa;
TaskHandle_t lora_receive_task_handle;
...
static Packet_router_args router_args = {
    ._LoRa = &myLoRa
};
...
int main(void)
{
  ...
  /* USER CODE BEGIN RTOS_THREADS */
   BaseType_t is_created = xTaskCreate(
        discover_nodes_task, // Task function
        "DiscoverNodes", // Task name
        256, // Stack size (adjust if needed)
        &discover_args, // Task arguments
        2, // Priority
        NULL // Optional task handle
    );

    // xTaskCreate(lora_receive_task,
    //     "lora_receive",
    //     256,
    //     &router_args,
    //     2,
    //     &lora_receive_task_handle
    //
    // );

    //TODO : create a lora receive task
